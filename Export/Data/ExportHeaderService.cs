...
namespace Export.Data {
    public class ExportHeaderService : IExportHeaderService {
        ...
        // This only works if you're already created the stored procedure.
        public async Task<bool> ExportHeaderUpdate(ExportHeader exportheader) {
            using (var conn = new SqlConnection(_configuration.Value)) {
                var parameters = new DynamicParameters();
                parameters.Add("Headerid", exportheader.Headerid, DbType.Int64);
        ...
        // This only works if you're already created the stored procedure.
        public async Task<bool> ExportHeaderDelete(int Headerid) {
            var parameters = new DynamicParameters();
            parameters.Add("@Headerid", Headerid, DbType.Int32);
            using (var conn = new SqlConnection(_configuration.Value)) {
                await conn.ExecuteAsync("spExportHeader_Delete", parameters, commandType: CommandType.StoredProcedure);
            }
            return true;
        }
    }
